...
const T = {
  el: {
    ...
    goalPlaceholder:"Τίτλος στόχου...", duePlaceholder:"Προθεσμία",
    ...
    fixed:"Πάγια", afterFixed:"Μετά τα Πάγια",
    ...
    savingsOf:(r,tgt)=>`${r}% του εισοδήματος · στόχος ${tgt}%`,
    noExpenses:"Δεν υπάρχουν έξοδα", fixedTotal:"Σύνολο Παγίων",
    ...
    noDeadlines:"Δεν υπάρχουν deadlines αυτό τον μήνα", upcoming:"Επερχόμενα",
    profileTitle:"Προφίλ", displayName:"Εμφανιζόμενο Όνομα",
    changePassword:"Αλλαγή Password", updateProfile:"Ενημέρωση Προφίλ",
    profileSaved:"Αποθηκεύτηκε!", uploadAvatar:"Αλλαγή φωτογραφίας", savingsTarget:"Στόχος %",
  },
  en: {
    ...
    goalPlaceholder:"Goal title...", duePlaceholder:"Due date",
    ...
    fixed:"Fixed", afterFixed:"After Fixed",
    ...
    savingsOf:(r,tgt)=>`${r}% of income · target ${tgt}%`,
    noExpenses:"No expenses yet", fixedTotal:"Total Fixed",
    ...
    noDeadlines:"No deadlines this month", upcoming:"Upcoming",
    profileTitle:"Profile", displayName:"Display Name",
    changePassword:"Change Password", updateProfile:"Update Profile",
    profileSaved:"Saved!", uploadAvatar:"Change photo", savingsTarget:"Target %",
  },
};
...
function FadeCard({ children, style, className, onClick }) {
  ...
  return <div className={className} onClick={onClick} style={{ ...style, opacity:vis?1:0, transform:vis?"translateY(0)":"translateY(10px)", transition:"opacity 0.3s ease,transform 0.3s ease,box-shadow 0.18s,border-color 0.18s" }}>{children}</div>;
}
...
function GoalsTab({ theme, isDark, lang, goals, setGoals }) {
  ...
  const [newGoal, setNewGoal] = useState({ title:"", areaIdx:0, priority:"high", due:"" });
  ...
  function addGoal() {
    ...
    setNewGoal({ title:"", areaIdx:0, priority:"high", due:"" });
    setAdding(false);
  }
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:14, height:"100%", minHeight:0 }}>
      ...
      <div style={{ display:"flex", gap:8, flexWrap:"wrap", alignItems:"center", flexShrink:0 }}>
        <button onClick={()=>setFilter("all")} style={{ padding:"6px 14px", borderRadius:20, border:`1px solid ${filter==="all"?"#60a5fa":theme.border}`, background:filter==="all"?"#60a5fa22":"transparent", color:filter==="all"?"#60a5fa":theme.textMuted, fontSize:13, fontFamily:"inherit", cursor:"pointer", fontWeight:filter==="all"?600:400 }}>{t.allAreas}</button>
      ...
      <div style={{ flex:1, minHeight:0, display:"grid", gridTemplateColumns:sel?"1fr min(380px,42%)":"1fr", gap:16, transition:"grid-template-columns 0.3s" }}>
        ...
        <div style={{ overflowY:"auto", minHeight:0, WebkitOverflowScrolling:"touch", overscrollBehavior:"contain" }}>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))", gap:14, alignContent:"start", paddingBottom:16 }}>
            {adding && (
                ...
                <FadeCard key={goal.id} className="goal-card" onClick={()=>{ setSelected(isSel?null:goal); setEditing(false); }}
                  ...
                  <div style={{ display:"flex", alignItems:"center", gap:14, marginBottom:14 }}>
                    ...
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontSize:15, fontWeight:600, color:theme.text, lineHeight:1.35, marginBottom:6 }}>{goalTitle(goal)}</div>
                      <div style={{ display:"flex", gap:5, flexWrap:"wrap" }}>
                        <span style={{ fontSize:11, padding:"2px 8px", borderRadius:10, background:color+"20", color, fontWeight:500 }}>{areaName(goal.areaIdx)}</span>
                        <span style={{ fontSize:11, padding:"2px 8px", borderRadius:10, background:goal.priority==="high"?"#f8717120":goal.priority==="medium"?"#fbbf2420":"#34d39920", color:goal.priority==="high"?"#f87171":goal.priority==="medium"?"#fbbf24":"#34d399", fontWeight:500 }}>{t.priority[goal.priority]}</span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div style={{ fontSize:12, color:isOverdue?"#f87171":theme.textMuted }}>
        ...
        {sel && (
          <div style={{ overflowY:"auto", minHeight:0, WebkitOverflowScrolling:"touch" }}>
            <div style={{ background:theme.surface, border:`1px solid ${AREA_COLORS[areaName(sel.areaIdx)]||theme.border}44`, borderRadius:16, padding:24, animation:"slideIn 0.22s ease" }}>
              ...
              {editing ? (
                <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
                  <input value={editDraft.title} onChange={e=>setEditDraft({...editDraft,title:e.target.value})} style={{ ...iStyle, fontSize:17, fontWeight:600 }}/>
                  ...
                  <div style={{ display:"flex", gap:8, marginTop:6 }}>
                    ...
                    <button onClick={()=>setEditing(false)} style={{ padding:"10px 14px", background:theme.input, border:`1px solid ${theme.borderStrong}`, borderRadius:8, color:theme.textMuted, fontSize:13, fontFamily:"inherit", cursor:"pointer" }}>{t.cancel}</button>
                  </div>
                ...
                <>
                  <div style={{ fontFamily:"'Syne',sans-serif", fontSize:21, fontWeight:800, color:theme.text, lineHeight:1.25, marginBottom:20 }}>{goalTitle(sel)}</div>
                  <div style={{ display:"flex", justifyContent:"center", marginBottom:20 }}>
                    <ProgressRing progress={sel.progress} color={AREA_COLORS[areaName(sel.areaIdx)]||"#60a5fa"} size={100} theme={theme}/>
                  </div>
                  <div style={{ marginBottom:18 }}>
                    <div style={{ fontSize:11, color:theme.textMuted, marginBottom:8, letterSpacing:"0.15em", textTransform:"uppercase", fontWeight:600 }}>{t.progressLabel}</div>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:16 }}>
                    {[
                      ...
                      <div key={m.label} style={{ background:theme.input, borderRadius:10, padding:"10px 14px", border:`1px solid ${theme.border}` }}>
                        <div style={{ fontSize:11, color:theme.textMuted, textTransform:"uppercase", letterSpacing:"0.12em", marginBottom:4, fontWeight:600 }}>{m.label}</div>
                        <div style={{ fontSize:14, color:theme.text, fontWeight:500 }}>{m.value}</div>
                      </div>
                    ))}
                  </div>
                  {sel.tasks?.length>0 && (
                    <div>
                      <div style={{ fontSize:11, color:theme.textMuted, letterSpacing:"0.15em", textTransform:"uppercase", marginBottom:10, fontWeight:600 }}>{t.stepsLabel}</div>
                      {sel.tasks.map((task,i)=>(
                        <div key={i} style={{ display:"flex", alignItems:"center", gap:12, padding:"9px 0", borderBottom:`1px solid ${theme.border}` }}>
                          <div style={{ width:6, height:6, borderRadius:"50%", background:AREA_COLORS[areaName(sel.areaIdx)]||"#60a5fa", flexShrink:0 }}/>
                          <span style={{ fontSize:13, color:theme.textMuted }}>{task}</span>
                        </div>
                      ))}
                    </div>
                  )}
...
// ─── CALENDAR TAB ─────────────────────────────────────────────────────────────
function CalendarTab({ theme, isDark, lang, goals, setTab }) {
  const t = T[lang];
  ...
  // Mon-first day names, single char
  const dayNames = lang==="el" ? ["Δ","Τ","Τ","Π","Π","Σ","Κ"] : ["M","T","W","T","F","S","S"];
  ...
  return (
    <div style={{ height:"100%", minHeight:0, overflowY:"auto", WebkitOverflowScrolling:"touch", overscrollBehavior:"contain", display:"flex", flexDirection:"column", gap:14 }}>

      {/* Month nav */}
      <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexShrink:0 }}>
        <button onClick={()=>setViewDate(new Date(year,month-1,1))} style={{ background:theme.input, border:`1px solid ${theme.border}`, borderRadius:8, color:theme.text, padding:"6px 16px", fontFamily:"inherit", cursor:"pointer", fontSize:20 }}>‹</button>
        <div style={{ fontFamily:"'Syne',sans-serif", fontSize:18, fontWeight:700, color:theme.text, textTransform:"capitalize" }}>{monthName}</div>
        <button onClick={()=>setViewDate(new Date(year,month+1,1))} style={{ background:theme.input, border:`1px solid ${theme.border}`, borderRadius:8, color:theme.text, padding:"6px 16px", fontFamily:"inherit", cursor:"pointer", fontSize:20 }}>›</button>
      </div>

      {/* Day-of-week headers */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:3, flexShrink:0 }}>
        {dayNames.map((d,i)=><div key={i} style={{ textAlign:"center", fontSize:11, color:theme.textMuted, fontWeight:600 }}>{d}</div>)}
      </div>

      {/* Compact calendar grid — goal title chips inside each cell */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(7,1fr)", gap:3, flexShrink:0 }}>
        {Array.from({ length:(firstDay+6)%7 }).map((_,i)=><div key={`e${i}`}/>)}
        {Array.from({ length:daysInMonth }).map((_,i)=>{
          const day = i+1;
          const isToday = today.getDate()===day && today.getMonth()===month && today.getFullYear()===year;
          const gs = goalsByDay[day];
          const isPast = new Date(year,month,day) < new Date(today.getFullYear(),today.getMonth(),today.getDate()) && !isToday;
          return (
            <div key={day} onClick={()=>gs&&setTab("goals")}
              style={{ minHeight: gs ? 62 : 30, display:"flex", flexDirection:"column", padding:"3px 4px 4px", borderRadius:8,
                background: isToday?"#2563eb" : gs?(isDark?"#1a2744":"#dbeafe") : "transparent",
                border:`1px solid ${isToday?"#3b82f6":gs?theme.borderStrong:"transparent"}`,
                cursor:gs?"pointer":"default", overflow:"hidden", transition:"all 0.15s" }}>
              <span style={{ fontSize:11, fontWeight:isToday?700:400, color:isToday?"#fff":isPast?theme.textDim:theme.text, lineHeight:1.4 }}>{day}</span>
              {gs && gs.slice(0,3).map(g=>{
                const color=AREA_COLORS[areaName(g.areaIdx)]||"#60a5fa";
                return (
                  <div key={g.id} style={{ marginTop:2, fontSize:9, fontWeight:600, color:"#fff", background:color, borderRadius:3, padding:"1px 4px", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap", lineHeight:1.6 }}>
                    {lang==="el"?g.titleEl:g.titleEn}
                  </div>
                );
              })}
              {gs && gs.length>3 && <div style={{ fontSize:8, color:theme.textMuted }}>+{gs.length-3}</div>}
            </div>
          );
        })}
      </div>
      ...
      {Object.keys(goalsByDay).length>0 && (
        <div style={{ flexShrink:0 }}>
          <div style={{ fontSize:11, color:theme.textMuted, fontWeight:600, letterSpacing:"0.1em", textTransform:"uppercase", marginBottom:8 }}>📋 Deadlines</div>
          {Object.entries(goalsByDay)
            .sort((a,b)=>Number(a[0])-Number(b[0]))
            .flatMap(([day,gs])=>gs.map(g=>{
              const color=AREA_COLORS[areaName(g.areaIdx)]||"#60a5fa";
              const dDate=new Date(year,month,Number(day));
              const daysLeft=Math.ceil((dDate-today)/(1000*60*60*24));
              const urgency=daysLeft<0?"#f87171":daysLeft<=3?"#f87171":daysLeft<=7?"#fbbf24":"#34d399";
              return (
                <div key={g.id} onClick={()=>setTab("goals")} style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 14px", background:theme.surface, border:`1px solid ${color}33`, borderRadius:10, marginBottom:6, cursor:"pointer" }}>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div style={{ fontSize:12, color:theme.textMuted }}>{dDate.toLocaleDateString(T[lang].dateLocale,{day:"numeric",month:"short"})} · {g.progress}%</div>
                  </div>
                  <span style={{ fontSize:11, padding:"2px 8px", borderRadius:8, background:urgency+"20", color:urgency, fontWeight:600, whiteSpace:"nowrap", flexShrink:0 }}>
                    {daysLeft<0?(lang==="el"?"Εκπρόθ.":"Overdue"):daysLeft===0?(lang==="el"?"Σήμερα":"Today"):`${daysLeft}d`}
                  </span>
                </div>
              );
            }))
      ...
      {Object.keys(goalsByDay).length===0 && <div style={{ textAlign:"center", padding:"12px 0", color:theme.textDim, fontSize:14 }}>{t.noDeadlines}</div>}

      {/* Upcoming panel — full width below */}
      {upcoming.length>0 && (
        <div style={{ background:theme.surface, border:`1px solid ${theme.border}`, borderRadius:14, padding:16, flexShrink:0 }}>
          <div style={{ fontSize:11, color:theme.textMuted, textTransform:"uppercase", letterSpacing:"0.14em", fontWeight:600, marginBottom:12 }}>🚀 {t.upcoming}</div>
          {upcoming.map(g=>{
            const color=AREA_COLORS[areaName(g.areaIdx)]||"#60a5fa";
            const daysLeft=Math.ceil((g.dueDate-today)/(1000*60*60*24));
            const urgency=daysLeft<=3?"#f87171":daysLeft<=7?"#fbbf24":"#34d399";
            return (
              <div key={g.id} style={{ display:"flex", alignItems:"center", gap:10, padding:"9px 0", borderBottom:`1px solid ${theme.border}` }}>
                <div style={{ flex:1, minWidth:0 }}>
                  <div style={{ fontSize:13, fontWeight:500, color:theme.text, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{lang==="el"?g.titleEl:g.titleEn}</div>
                  <div style={{ display:"flex", alignItems:"center", gap:8, marginTop:4 }}>
                    <div style={{ flex:1, height:3, background:theme.ring, borderRadius:2, overflow:"hidden" }}>
                      <div style={{ height:"100%", width:`${g.progress}%`, background:color, borderRadius:2 }}/>
                    </div>
                    <span style={{ fontSize:11, color:theme.textMuted, flexShrink:0 }}>{g.progress}%</span>
                  </div>
                </div>
                <span style={{ fontSize:11, padding:"2px 8px", borderRadius:8, background:urgency+"20", color:urgency, fontWeight:600, whiteSpace:"nowrap", flexShrink:0 }}>
...
function WhispererTab({ theme, isDark, lang, notes, setNotes }) {
  ...
  // Collapsible quick capture: hides on scroll down, reappears on scroll up
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      if (y > lastScrollY.current + 8) setCaptureCollapsed(true);
      else if (y < lastScrollY.current - 8) setCaptureCollapsed(false);
  ...
  function startEdit(e,note) { e.stopPropagation(); setEditingId(note.id); setEditContent(lang==="el"?note.content:(note.contentEn||note.content)); setEditTags(note.tags.join(", ")); }
  function saveEdit(e,note) { e.stopPropagation(); const tags=editTags.split(",").map(tg=>tg.trim()).filter(Boolean); setNotes(notes.map(n=>n.id===note.id?{...n,content:editContent,contentEn:editContent,tags}:n)); setEditingId(null); }
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:10, height:"100%", minHeight:0 }}>
      ...
      {/* Quick Capture — collapses to slim bar on scroll down */}
      <div style={{ flexShrink:0, overflow:"hidden",
        maxHeight: captureCollapsed ? 38 : 200,
        transition:"max-height 0.3s cubic-bezier(.4,0,.2,1), opacity 0.2s",
        opacity: captureCollapsed ? 0.8 : 1 }}>
        {captureCollapsed ? (
          <button onClick={()=>{ setCaptureCollapsed(false); }}
            style={{ width:"100%", padding:"8px 16px", background:theme.surface, border:`1px solid ${isDark?"#2d1b4e":"#e9d5ff"}`, borderRadius:12, color:theme.textMuted, fontSize:12, fontFamily:"inherit", cursor:"pointer", textAlign:"left", display:"flex", alignItems:"center", gap:8 }}>
            <span style={{ color:"#c084fc" }}>⚡</span>
            <span style={{ flex:1 }}>{t.capturePlaceholder}</span>
            <span style={{ fontSize:10 }}>↑</span>
          </button>
        ) : (
          <div style={{ background:theme.surface, border:`1px solid ${isDark?"#2d1b4e":"#e9d5ff"}`, borderRadius:16, padding:16 }}>
            <div style={{ fontSize:11, color:"#c084fc", letterSpacing:"0.18em", textTransform:"uppercase", marginBottom:8, fontWeight:600 }}>{t.quickCapture}</div>
            ...
            <div style={{ display:"flex", gap:8 }}>
              ...
              <button onClick={addNote} style={{ padding:"8px 16px", background:"#7c3aed", border:"none", borderRadius:8, color:"#ede9fe", fontSize:13, fontFamily:"inherit", cursor:"pointer", fontWeight:600, whiteSpace:"nowrap" }}>{t.save}</button>
            </div>
          </div>
        )}
      </div>
      ...
      <div style={{ display:"flex", gap:7, flexWrap:"wrap", alignItems:"center", flexShrink:0 }}>
        <div style={{ position:"relative" }}>
          ...
          <input placeholder={t.searchPlaceholder} value={search} onChange={e=>setSearch(e.target.value)}
            style={{ background:theme.surface, border:`1px solid ${theme.border}`, borderRadius:20, padding:"7px 14px 7px 30px", color:theme.text, fontSize:13, fontFamily:"inherit", width:160, outline:"none" }}/>
        </div>
        {allTags.map(tag=>(
      ...
      {/* Notes — scrollRef here drives the collapse logic */}
        ...
        <div style={{ columns:"290px", columnGap:14, paddingBottom:32 }}>
          {filtered.map((note,i)=>(
            <div key={note.id} className="note-card"
              style={{ breakInside:"avoid", marginBottom:14, background:note.pinned?theme.pinBg:theme.surface, border:`1px solid ${note.pinned?theme.pinBorder:theme.border}`, borderRadius:14, padding:18, position:"relative", boxShadow:note.pinned?`0 0 28px ${isDark?"#7c3aed18":"#a78bfa22"}`:"none", transition:"transform 0.18s", animation:"fadeUp 0.3s ease both", animationDelay:`${i*40}ms` }}>
              <div style={{ position:"absolute", top:12, right:12, display:"flex", gap:4 }}>
                <button onClick={e=>startEdit(e,note)} style={{ background:"none", border:"none", cursor:"pointer", fontSize:13, color:theme.textMuted, opacity:editingId===note.id?1:0.3, transition:"opacity 0.15s", padding:"2px 5px" }} onMouseEnter={e=>e.currentTarget.style.opacity="1"} onMouseLeave={e=>e.currentTarget.style.opacity=editingId===note.id?"1":"0.3"}>✏️</button>
...
function BudgetTab({ theme, isDark, lang, txns, setTxns, savingsTarget, setSavingsTarget }) {
  ...
  // Swipe state — panel 0=transactions, 1=breakdown
  ...
  const onTouchEnd = (e) => {
    ...
    // Only register horizontal swipes (dx > 40px, more horizontal than vertical)
    if (Math.abs(dx) > 40 && Math.abs(dx) > dy * 1.5) {
      if (dx < 0 && panel === 0) setPanel(1); // swipe left → breakdown
      if (dx > 0 && panel === 1) setPanel(0); // swipe right → transactions
    }
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:10, height:"100%", minHeight:0 }}>
      ...
      <div style={{ flex:1, minHeight:0, overflowY:"auto", WebkitOverflowScrolling:"touch", overscrollBehavior:"contain" }}>
        {filtered.map((txn)=>{
          ...
          return (
            <div key={txn.id} className="note-card" style={{ display:"flex", alignItems:"center", gap:12, padding:"11px 14px", background:theme.surface, border:`1px solid ${isFixed?"#f8717130":theme.border}`, borderRadius:12, marginBottom:7 }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                  ...
                  {isFixed && <span style={{ fontSize:10, padding:"1px 5px", borderRadius:5, background:"#f8717122", color:"#f87171", fontWeight:600, flexShrink:0 }}>🔒</span>}
                </div>
                <div style={{ fontSize:12, color:theme.textMuted, marginTop:1 }}>{cat} · {txn.date}</div>
              </div>
              <div style={{ fontSize:15, fontWeight:700, color:txn.type==="income"?"#34d399":isFixed?"#f87171":"#fb923c", fontFamily:"'Syne',sans-serif", flexShrink:0 }}>{txn.type==="income"?"+":""}{txn.amount.toFixed(0)}€</div>
  ...
  const BreakdownPanel = (
    <div style={{ overflowY:"auto", WebkitOverflowScrolling:"touch", overscrollBehavior:"contain", height:"100%" }}>
      <div style={{ background:theme.surface, border:`1px solid ${theme.border}`, borderRadius:14, padding:18 }}>
        {fixedTotal>0 && (
          <div style={{ marginBottom:16, padding:"12px 14px", background:isDark?"#1a0a0a":"#fff5f5", border:"1px solid #f8717130", borderRadius:10 }}>
            <div style={{ fontSize:11, color:"#f87171", textTransform:"uppercase", letterSpacing:"0.12em", fontWeight:600, marginBottom:8 }}>🔒 {t.fixedTotal}</div>
        ...
        <div style={{ fontSize:11, color:theme.textMuted, textTransform:"uppercase", letterSpacing:"0.12em", fontWeight:600, marginBottom:12 }}>{t.breakdown}</div>
          ...
          return (
            <div key={cat} style={{ marginBottom:12 }}>
              <div style={{ display:"flex", justifyContent:"space-between", marginBottom:4 }}>
                ...
                <span style={{ fontSize:12, color:theme.textMuted }}>{amt}€ <span style={{ fontSize:10, color:theme.textDim }}>({pct}%)</span></span>
              </div>
        ...
        {sortedCats.length===0 && <div style={{ fontSize:13, color:theme.textDim, textAlign:"center", padding:"12px 0" }}>{t.noExpenses}</div>}
        <div style={{ marginTop:14, paddingTop:14, borderTop:`1px solid ${theme.border}` }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:8 }}>
            <div style={{ fontSize:11, color:theme.textMuted, textTransform:"uppercase", letterSpacing:"0.12em", fontWeight:600 }}>{t.savingsGoal}</div>
            {editingTarget ? (
              <div style={{ display:"flex", gap:4, alignItems:"center" }}>
                <input type="number" min={1} max={100} value={targetDraft} onChange={e=>setTargetDraft(e.target.value)}
                  style={{ width:48, background:theme.input, border:`1px solid ${theme.borderStrong}`, borderRadius:6, padding:"3px 6px", color:theme.text, fontSize:12, fontFamily:"inherit", textAlign:"center" }}/>
                <span style={{ fontSize:12, color:theme.textMuted }}>%</span>
                <button onClick={()=>{ setSavingsTarget(Math.max(1,Math.min(100,parseInt(targetDraft)||20))); setEditingTarget(false); }} style={{ background:"#2563eb", border:"none", borderRadius:6, color:"#fff", fontSize:11, padding:"3px 8px", cursor:"pointer" }}>✓</button>
              </div>
            ) : (
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:12, height:"100%", minHeight:0 }}>
      ...
      {/* Summary cards */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(100px,1fr))", gap:8, flexShrink:0 }}>
        {[
          ...
          <div key={s.label} style={{ background:theme.surface, border:`1px solid ${s.badge?s.color+"44":theme.border}`, borderRadius:12, padding:"10px 12px", position:"relative" }}>
            {s.badge && <span style={{ position:"absolute", top:6, right:8, fontSize:10 }}>{s.badge}</span>}
            <div style={{ fontSize:10, color:theme.textMuted, textTransform:"uppercase", letterSpacing:"0.08em", fontWeight:600, marginBottom:4 }}>{s.label}</div>
            <div style={{ fontSize:17, fontWeight:800, fontFamily:"'Syne',sans-serif", color:s.color }}>{s.value.toFixed(0)}€</div>
          </div>
        ))}
      </div>

      {/* ── DESKTOP: side-by-side grid ── */}
      <div className="budget-desktop" style={{ flex:1, minHeight:0, display:"grid", gridTemplateColumns:"1fr 260px", gap:14 }}>
        {TransactionsPanel}
        {BreakdownPanel}
      </div>

      {/* ── MOBILE: swipeable panels ── */}
      <div className="budget-mobile" style={{ display:"none", flex:1, minHeight:0, flexDirection:"column", gap:10 }}>
        {/* Dot indicator + panel labels */}
        <div style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:16, flexShrink:0 }}>
          <button onClick={()=>setPanel(0)} style={{ background:"none", border:"none", cursor:"pointer", display:"flex", alignItems:"center", gap:6, color:panel===0?"#60a5fa":theme.textMuted, fontSize:12, fontFamily:"inherit", fontWeight:panel===0?600:400, padding:0 }}>
            💸 {t.allFilter}
          </button>
          <div style={{ display:"flex", gap:6 }}>
            <div style={{ width:6, height:6, borderRadius:"50%", background:panel===0?"#60a5fa":theme.textDim, transition:"background 0.2s" }}/>
            <div style={{ width:6, height:6, borderRadius:"50%", background:panel===1?"#60a5fa":theme.textDim, transition:"background 0.2s" }}/>
          </div>
          <button onClick={()=>setPanel(1)} style={{ background:"none", border:"none", cursor:"pointer", display:"flex", alignItems:"center", gap:6, color:panel===1?"#60a5fa":theme.textMuted, fontSize:12, fontFamily:"inherit", fontWeight:panel===1?600:400, padding:0 }}>
            📊 {t.breakdown}
          </button>
        </div>
        ...
        <div ref={swipeRef} onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}
          ...
          {/* Sliding track */}
          <div style={{ display:"flex", width:"200%", height:"100%", transform:`translateX(${panel===0?0:-50}%)`, transition:"transform 0.32s cubic-bezier(.4,0,.2,1)" }}>
            <div style={{ width:"50%", height:"100%", flexShrink:0, paddingRight:6 }}>
              {TransactionsPanel}
            </div>
            <div style={{ width:"50%", height:"100%", flexShrink:0, paddingLeft:6 }}>
              {BreakdownPanel}
            </div>
          </div>
        </div>
        ...
        <div style={{ textAlign:"center", fontSize:11, color:theme.textDim, flexShrink:0 }}>
          ← {lang==="el"?"σύρε για ανάλυση":"swipe for breakdown"} →
        </div>
      </div>
...
function ProfileTab({ theme, isDark, lang, session }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (newPass) {
        if (newPass!==confirmPass) { setError(lang==="el"?"Τα passwords δεν ταιριάζουν":"Passwords don't match"); setSaving(false); return; }
  ...
  async function handleAvatar(e) {
    ...
    try {
      ...
      setAvatarUrl(url); setMsg(t.profileSaved);
    } catch(e) { setError(e.message); }
    setSaving(false);
  }
  ...
  return (
    <div style={{ height:"100%", minHeight:0, overflowY:"auto", WebkitOverflowScrolling:"touch" }}>
      <div style={{ maxWidth:460, margin:"0 auto", paddingBottom:40 }}>
        <div style={{ fontSize:11, color:theme.textMuted, letterSpacing:"0.18em", textTransform:"uppercase", fontWeight:600, marginBottom:24 }}>{t.profileTitle}</div>

        {/* Avatar */}
        <div style={{ display:"flex", flexDirection:"column", alignItems:"center", marginBottom:28 }}>
          <div onClick={()=>fileRef.current?.click()} style={{ width:90, height:90, borderRadius:"50%", background:avatarUrl?"transparent":(isDark?"#1e3a8a":"#dbeafe"), border:`2px solid ${theme.borderStrong}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:32, fontWeight:700, fontFamily:"'Syne',sans-serif", color:"#60a5fa", overflow:"hidden", marginBottom:10, cursor:"pointer" }}>
          ...
          <div style={{ marginTop:5, fontSize:13, color:theme.textMuted }}>{session.user.email}</div>
        </div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          <div style={{ borderTop:`1px solid ${theme.border}`, paddingTop:18, marginTop:4 }}>
            <div style={{ fontSize:11, color:theme.textMuted, letterSpacing:"0.14em", textTransform:"uppercase", fontWeight:600, marginBottom:12 }}>{t.changePassword}</div>
            <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          ...
          <button onClick={handleSave} disabled={saving} style={{ padding:"13px", background:saving?"#1e3a8a":"#2563eb", border:"none", borderRadius:10, color:"#fff", fontSize:14, fontFamily:"inherit", fontWeight:600, cursor:saving?"not-allowed":"pointer" }}>
    ...
    <div style={{ display:"flex", gap:16 }}>
      {[{label:t.goals,value:goalCount,color:"#60a5fa"},{label:t.notes,value:noteCount,color:"#a78bfa"}].map(s=>(
        <div key={s.label} style={{ textAlign:"right" }}>
          <div style={{ fontSize:22, fontWeight:800, fontFamily:"'Syne',sans-serif", color:s.color, lineHeight:1 }}>{s.value}</div>
          <div style={{ fontSize:10, color:theme.textMuted, letterSpacing:"0.1em", textTransform:"uppercase", marginTop:2, fontWeight:500 }}>{s.label}</div>
        </div>
      ))}
    </div>
  );
...
function SecondBrain({ session }) {
  ...
  const [tab,    setTabRaw]    = useState(()=>{ const l=localStorage.getItem("sb_tab"); return l||"goals"; });
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:theme.bg, fontFamily:"'DM Mono','Courier New',monospace", color:theme.text, display:"flex", flexDirection:"column", overflow:"hidden", transition:"background 0.3s,color 0.3s" }}>
      <style>{`
        ...
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        @media (max-width:640px) {
          ...
          .cpad { padding:10px 14px 0 !important; }
          ::-webkit-scrollbar { width:0px; }
          /* Budget: hide desktop grid, show swipeable on mobile */
          .budget-desktop { display:none !important; }
          .budget-mobile  { display:flex !important; }
        }
        @media (min-width:641px) {
          .mobile-nav    { display:none !important; }
          .budget-mobile { display:none !important; }
          .hpad { padding:20px 40px 0; }
          .tpad { padding:14px 40px 0; }
          .cpad { padding:16px 40px 20px; }
        }
      `}</style>
      ...
      {/* ── HEADER ── */}
      <div className="hpad" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", flexWrap:"wrap", gap:8, flexShrink:0 }}>
        <div>
          <div style={{ fontSize:11, letterSpacing:"0.12em", color:theme.textMuted, marginBottom:2, textTransform:"uppercase", fontWeight:500 }}>
          ...
          <h1 style={{ fontFamily:"'Syne',sans-serif", lineHeight:1.05, color:theme.text }}>
            <span style={{ fontSize:16, fontWeight:700, opacity:0.55 }}>{t.appTitle[0]} </span>
            <span style={{ fontSize:30, fontWeight:800, color:"#60a5fa" }}>{t.appTitle[1]}</span>
          </h1>
        </div>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8, flexWrap:"wrap" }}>
          ...
          <button onClick={()=>setLang(lang==="el"?"en":"el")} style={{ padding:"5px 10px", borderRadius:20, border:`1px solid ${theme.border}`, background:theme.surface, color:theme.text, fontSize:12, fontFamily:"inherit", fontWeight:600, display:"flex", alignItems:"center", gap:4 }}>
            <span>{lang==="el"?"🇬🇷":"🇬🇧"}</span><span style={{ fontSize:10, color:theme.textMuted }}>{lang==="el"?"EN":"ΕΛ"}</span>
          </button>

          <button onClick={()=>setIsDark(!isDark)} style={{ width:42, height:22, borderRadius:11, background:isDark?"#1e3a8a":"#e0e7ff", border:`1px solid ${isDark?"#3b82f6":"#a5b4fc"}`, cursor:"pointer", position:"relative", transition:"background 0.3s", flexShrink:0 }}>
            <div style={{ position:"absolute", top:2, left:isDark?2:19, width:16, height:16, borderRadius:"50%", background:isDark?"#60a5fa":"#6366f1", transition:"left 0.25s cubic-bezier(.4,0,.2,1)", display:"flex", alignItems:"center", justifyContent:"center", fontSize:9 }}>{isDark?"🌙":"☀️"}</div>
          </button>

          {/* Profile button */}
          <button onClick={()=>setTab("profile")} style={{ width:32, height:32, borderRadius:"50%", background:tab==="profile"?"#2563eb":theme.input, border:`2px solid ${tab==="profile"?"#2563eb":theme.borderStrong}`, cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", fontSize:14, transition:"all 0.18s", flexShrink:0, overflow:"hidden" }}>
            {session.user.user_metadata?.avatar_url
              ? <img src={session.user.user_metadata.avatar_url} style={{ width:"100%", height:"100%", objectFit:"cover" }}/>
              : "👤"}
          </button>

          <button onClick={()=>auth.signOut()} style={{ padding:"5px 12px", borderRadius:20, border:`1px solid ${theme.border}`, background:"transparent", color:theme.textMuted, fontSize:11, fontFamily:"inherit", cursor:"pointer", transition:"all 0.18s" }}
      ...
      {/* ── DESKTOP TABS ── */}
      <div className="tpad desktop-tabs" style={{ display:"flex", gap:4, flexShrink:0 }}>
        {TABS.map(id=>(
          <button key={id} onClick={()=>setTab(id)} style={{ padding:"7px 16px", border:`1px solid ${theme.border}`, borderBottom:tab===id?`1px solid ${theme.bg}`:`1px solid ${theme.border}`, borderRadius:"10px 10px 0 0", background:tab===id?theme.surface:"transparent", color:tab===id?theme.text:theme.textMuted, fontSize:13, fontFamily:"inherit", fontWeight:tab===id?500:400, transition:"all 0.18s" }}>
            {t.tabs[id]}
          </button>
        ))}
        {tab==="profile" && (
          <button style={{ padding:"7px 16px", border:`1px solid ${theme.border}`, borderBottom:`1px solid ${theme.bg}`, borderRadius:"10px 10px 0 0", background:theme.surface, color:theme.text, fontSize:13, fontFamily:"inherit", fontWeight:500 }}>{t.tabs.profile}</button>
        )}
        <div style={{ flex:1, borderBottom:`1px solid ${theme.border}` }}/>
      </div>

      {/* ── CONTENT ── */}
      <div className="cpad" style={{ flex:1, minHeight:0, display:"flex", flexDirection:"column" }}>
      ...
      {/* ── MOBILE BOTTOM NAV ── */}
      <div className="mobile-nav" style={{ display:"none", position:"fixed", bottom:0, left:0, right:0, background:theme.surface, borderTop:`1px solid ${theme.border}`, padding:"6px 0 calc(6px + env(safe-area-inset-bottom,0px))", zIndex:50, justifyContent:"space-around", alignItems:"center" }}>
        {[...TABS,"profile"].map(id=>{
          const icons={ goals:"🎯", whisperer:"🌀", budget:"💸", calendar:"📅", profile:"👤" };
          const labels={ goals:t.goals, whisperer:"Whisperer", budget:"Budget", calendar:"Calendar", profile:"Me" };
          const active=tab===id;
          return (
            <button key={id} onClick={()=>setTab(id)}
              style={{ display:"flex", flexDirection:"column", alignItems:"center", gap:2, background:"none", border:"none", padding:"4px 10px", borderRadius:10, cursor:"pointer", color:active?"#60a5fa":theme.textMuted, minWidth:48 }}>
              {id==="profile"&&session.user.user_metadata?.avatar_url
                ? <img src={session.user.user_metadata.avatar_url} style={{ width:22, height:22, borderRadius:"50%", objectFit:"cover", border:`2px solid ${active?"#60a5fa":theme.border}` }}/>
                : <span style={{ fontSize:20 }}>{icons[id]}</span>}
              <span style={{ fontSize:10, fontWeight:active?600:400 }}>{labels[id]}</span>
              {active && <div style={{ width:3, height:3, borderRadius:"50%", background:"#60a5fa" }}/>}
            </button>
          );
        })}
      </div>

      {/* Spacer so content isn't behind mobile nav */}
      <div className="mobile-nav" style={{ display:"none", height:65, flexShrink:0 }}/>
